...
// Extern symbol for embedded certificate
...
// Encode raw image data to Base64
...
char* server_send_image(camera_fb_t* fb) {
    ...
    esp_http_client_config_t config = {
        ...
        .skip_cert_common_name_check = false,           // 🔐 Set to false for safer validation
    };
    ...
    if (err == ESP_OK) {
        ...
        if (status == 200 && content_length > 0) {
            ...
            if (response_buffer) {
                int read = esp_http_client_read(client, response_buffer, content_length);
                if (read > 0) {
